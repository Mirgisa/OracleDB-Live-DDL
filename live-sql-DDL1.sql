...
SELECT F.TITOLO, SUM(P.INCASSO) AS TOTALEINCASSI
FROM FILM F
JOIN PROIEZIONI P ON F.CODFILM = P.CODFILM
WHERE F.GENERE = 'SCIENCE FICTION'
GROUP BY F.TITOLO;
...
FROM FILM F
JOIN PROIEZIONI P ON F.CODFILM = P.CODFILM
WHERE F.GENERE = 'SCIENCE FICTION' AND P.DATAPROIEZIONE > '2001-01-01'
GROUP BY F.TITOLO;
...
FROM FILM F
JOIN PROIEZIONI P ON F.CODFILM = P.CODFILM
WHERE F.GENERE = 'SCIENCE FICTION' AND F.CODFILM NOT IN
        (SELECT CODFILM FROM PROIEZIONI WHERE DATAPROIEZIONE < '2001-01-01')
GROUP BY F.TITOLO;
...
SELECT S.NOME, SUM(P.INCASSO) AS TOTALEINCASSI
FROM SALE S
...
AND A.CODATTORE NOT IN
       ...
       WHERE F.GENERE = 'SCIENCE FICTION' AND F.REGISTA = 'F.FELLINI');
...
WHERE A.CODATTORE IN
       ...
       FROM RECITA R
       JOIN FILM F ON R.CODFILM = F.CODFILM
       WHERE F.REGISTA = 'F. FELLINI' AND F.ANNOPRODUZIONE < 1960)
	AND A.CODATTORE NOT IN
      (SELECT R.CODATTORE
      FROM RECITA R
      JOIN FILM F ON R.CODFILM = F.CODFILM
      WHERE F.REGISTA <> 'F. FELLINI' OR F.ANNOPRODUZIONE >= 1960);
